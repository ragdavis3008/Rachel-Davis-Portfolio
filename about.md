# About Me

Hi, I'm **Rachel A.G. Davis** — an experienced cross-functional business professional based in Niederkassel, Germany, with over 20 years of professional experience and significant longevity with the organizations I've worked for.

## Professional Background

My career has spanned business development, sales, project management, customer success, service delivery, operations, administration, employee and customer training, process improvement, and CRM management. I have worked across different departments and functions throughout my career, which has given me a strong overall perspective on how departments depend on one another — and how sales, operations, service, project delivery, administration, and customer-facing teams need to communicate and work together for an organization to succeed.

I have been responsible for both hardware and software projects, including project coordination, implementation, customer communication, technical-team coordination, service delivery, training, and post-sale customer management.

I'm also expanding my experience into AI, learning how to build applications, AI-assisted workflows, and automations using AI agents and Claude Code.

## Skills

- Business Development
- Project Management
- Hardware Project Management
- Software Project Management
- Customer Success
- Service Delivery Management
- Operations Management
- Multi-Department Operations Management
- Cross-Functional Leadership and Coordination
- Process Improvement
- Workflow Design
- CRM Administration
- CRM Migration
- AI Automation and AI-Assisted Workflows
- Employee Onboarding
- Employee Training
- Internal Training
- Customer Training
- Knowledge Base Development and Management
- Training Program Development
- Documentation and SOP Development
- RFP and Proposal Management
- Public-Sector Bids
- Customer Relationship Management
- Implementation and Post-Sale Project Management
- Stakeholder Communication

## Technologies

### CRM, ERP & Business Systems
- Monday.com
- Salesforce
- QuickBooks
- OpenZ
- Microsoft SharePoint
- Monday.com Project Management
- Microsoft 365

### AI & Development Tools
- Claude Code
- Claude
- ChatGPT
- AI Agent Tools
- VS Code
- Git
- GitHub

### Microsoft & Productivity
- Microsoft 365
- Excel
- Word
- PowerPoint
- Outlook
- Teams
- OneDrive
- SharePoint

### Google Workspace
- Google Docs
- Google Sheets
- Google Drive
- Gmail
- Google Meet
- Google Slides
- Google Chat

### Collaboration
- Zoom
- Slack
- Discord
- Monday.com

### Currently Learning
- Building AI-Powered Applications, Automations, and Workflows with AI Agents (Turing College)
- HTML
- CSS
- JavaScript
- Markdown
- Git/GitHub Version Control
- Prompt Engineering
- AI Agents
- AI-Assisted Workflow Automation

## Key Accomplishments

- Independently drove sales growth from approximately $500,000 to $2 million within eight months.
- Managed multiple customer projects simultaneously, from sale through implementation and final delivery.
- Managed both hardware and software projects involving customers, technical teams, implementation, training, service, and post-sale support.
- Led the company's CRM migration from Salesforce to Monday.com, including data mapping, process design, and coordination with developers to create a smooth transition.
- Implemented Monday.com across the organization, served as system administrator, and oversaw workflows, user training, process improvements, and AI/automation initiatives.
- Developed and improved internal business processes and customer-facing processes.
- Developed internal and customer documentation, training materials, and knowledge-base resources.
- Managed employee onboarding and internal training.
- Worked across sales, customer service, project delivery, operations, and administration, helping improve coordination between departments.
- Currently learning to build applications, AI-assisted workflows, and automations using AI agents and Claude Code.

## Work Experience

### mk Solutions GmbH — Germany
**Business Development & Project Manager** · 2019–2025

Led business development, project and implementation management, customer relationships, service delivery, and operational initiatives across hardware and software solutions. Drove CRM and process implementations, AI-enabled workflow automations, operational improvements, and cross-functional coordination to support growth, efficiency, and customer delivery.

### mk Solutions, Inc. — USA
**Sales, Project and Office Manager** · 2013–2019

Managed service department operations, project delivery, office and business operations, customer coordination, sales support, implementations, and internal process improvement. Oversaw service scheduling and workflow coordination, employee onboarding, training and documentation, customer communications, and cross-functional initiatives supporting both day-to-day operations and long-term business growth.

### Texas Gulf Industries, Inc.
**Head of Business Development & Operations / Vice President** · 1999–2013

Directed day-to-day business operations across all functional areas, including project and implementation management, office operations, customer coordination, sales support, service delivery, internal processes, employee onboarding and training, and cross-functional execution. Served as a central point of coordination across the business, helping manage priorities, resolve operational issues, support customers and staff, and keep projects and daily operations moving efficiently. Also managed EPA registrations, compliance requirements, and related documentation to ensure the business met applicable regulatory obligations.

### Black & Decker
**Accounts & Channel Manager** · 1997–1999

Led sales and marketing activities, including customer presentations, product demonstrations, promotional planning, and campaign execution. Worked closely with production teams to coordinate delivery timelines, support demand forecasting, and align customer commitments with manufacturing capacity. Developed and launched original sales promotions and marketing initiatives to drive product visibility, customer engagement, and revenue growth.

### Marriott Corporation
**Sales and Conference Services Manager** · 1990–1997

Managed both group sales and conference services, supporting clients from initial inquiry through event execution. Coordinated contracts, signatures, meeting documentation, room and event schedules, audiovisual requirements, catering details, and cross-department communication. Oversaw employee scheduling, customer relationships, and day-to-day conference service operations, working closely with banquet, catering, front office, housekeeping, and other hotel teams to ensure successful events and a high level of guest service.

## Education & Professional Development

### Turing College
**Building with AI Agents** · Expected completion: December 2026

Current professional development focused on using AI agents to build applications, automate workflows, and create practical AI-assisted solutions.

### Stevenson University (formerly Villa Julie College)
1989–1992 · Business Administration / Mass Communication coursework

### Towson University
1994–1995 · Business Administration / Mass Communication coursework

## Location

Niederkassel, Germany

## Contact

- **Location:** Niederkassel, Germany
- **Email:** ragdavis30082@gmail.com
- **LinkedIn:** https://www.linkedin.com/in/rachelagdavis30082/
